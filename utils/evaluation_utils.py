...
def average_recall(target, prediction):
    unique_classes = np.union1d(np.unique(target), np.unique(prediction))
    recall_scores = []

    for cls in unique_classes:
        if cls == 0:  # Assuming class 0 is background and should be ignored
            continue
...
def average_precision(target, prediction):
    unique_classes = np.union1d(np.unique(target), np.unique(prediction))
    precision_scores = []
